...
Utils util = Utils();
float cameraHeight = 1.0f, cameraPitch = 15.0f;
...
double turbulence(double x, double y, double z, double maxZoom) {
	...
	sum = (sin((1.0/512.0)*(8*PI)*(x+z-4*y)) + 1) * 8.0 + (cos((1.0 / 512.0) * (8 * PI) * (x + z - 4 * y)) + 1) * 8.0;
...
int main(void) {
	...
	GLFWwindow* window = glfwCreateWindow(600, 600, "Waterwave Simulation", NULL, NULL);
